  ...
  class GoodsList {
    ...
    filter(){
      document.getElementById('search').addEventListener('keydown', (event) =>{
        ...
        let regexMouse = /мышка/i
        let regexNote = /ноутбук/i

        if(regexMouse.test(value)){
          this.useRegEx(regexMouse)
        }
        if(regexNote.test(value)){
          this.useRegEx(regexNote)
        }
        else {
          return;
        }
    ...
    useRegEx(regEx){
      let arr = this.goods.filter(elem => {
        if(regEx.test(elem.product_name)){
          return true;
        }
        else {
          return false
